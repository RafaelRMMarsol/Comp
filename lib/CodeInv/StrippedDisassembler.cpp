...
void StrippedDisassembler::functionsIterator(uint64_t Address) {
  ...
  for (; BI != BE; ++BI) {
    ...
    if (BI->instr_rbegin()->isBranch()) {
      uint64_t InstAddr =
          DAS->getDebugOffset(BI->instr_rbegin()->getDebugLoc());
      uint64_t InstSize = (TripleName.find("arm") != std::string::npos)
                              ? 8
...
void StrippedDisassembler::findStrippedMain() {
  ...
  else if ((TripleName.find("i386") != std::string::npos) ||
           (TripleName.find("x86") != std::string::npos)) {
    while (BI != BE) {
      if (II->getOpcode() >= 8472 && II->getOpcode() <= 8511) break;
      ++II;
      if (II == IE) {
        ++BI;
        II = BI->instr_begin();
...
StrippedGraph *StrippedDisassembler::getStrippedGraph() { return Graph; }
uint64_t StrippedDisassembler::getMain() { return mainAddr; }
